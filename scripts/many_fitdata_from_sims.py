...
parser.add_argument("-trials", dest='num_trials', help='trials per worker', 
    type=int, default=1 )
parser.add_argument("--tilt-plane-int", dest='tilt_plane_int', action='store_true')
args = parser.parse_args()

tilt_plane_integration=args.tilt_plane_int
        ...
        for pid in rpp:
            ...
            if inA and inB:  # are there both A and B channel reflections ? If so, lets find out which ones have same hkl
                ...
                for p in pointsA:
                    ...
                    q = QAmag[iiy,iix]
                    radA = detdist * np.tan(2*np.arcsin(q*waveA/4/np.pi)) / pixsize
                    radB = detdist * np.tan(2*np.arcsin(q*waveB/4/np.pi)) / pixsize
