...
basedir = "/reg/d/psdm/cxi/cxid9114/scratch/dermen/idx/mad/"
exp_name = "%s/results/run%d/exp_%d_feb8th.json" % (basedir,run, shot_idx)
data_name = "%s/results/run%d/dump_%d_feb8th.pkl" % (basedir,run, shot_idx)
...
def test(N=150):
    ...
    for i_trial in range(N):
        ...
        new_shape = JitterFactory.jitter_shape(
            min_Ncell=min_Ncell, max_Ncell=max_Ncell, min_mos_spread=0.005, max_mos_spread=0.1)
...
master_img = [s/ s.max() for s in master_img]
#master_img /= master_img.max()

#thresh = 1e-3
#master_img2 = np.zeros_like( master_img)
#for i_m,m in enumerate(master_img):
#    lab_im, nlab = label( m > thresh ) 
#    for lab in np.arange(1, nlab+1):
#        m[lab_im==lab] = m[lab_im==lab] / m[lab_im==lab].max() 
#    master_img2[i_m] = m
#    master_img2[ i_m,lab_im==0] = 0
#
#master_img = master_img2
...
for refl in refls_strong:
    ...
    tota_proba = proba_pix[ proba_pix > 0].mean()
