...
class eigen_helper(cxid9114.log_sparse_jac_base,levenberg_common,normal_eqns.non_linear_ls_mixin):
  ...
  def build_up(self, objective_only=False):
    ...
    self.reset()
    if not objective_only:
      functional = self.functional_basic(self.x)
      print("\n\t<><><><><><>")
      print("\tFunctional value: %.10e" % functional)
      if self.truth is not None:
        functional_ideal = self.functional_basic(self.truth)
    ...
    self.barf(objective_only, current_values = self.x)
...
class eigen_solver(solvers.LBFGSsolver):
  ...
  def __init__(self, conj_grad=True, weights=None, plot_truth=False,
    ...
    if sovlerization_maximus:
        ...
        print "Converged functional: ", self.helper.functional_basic(self.helper.x)
...
def simdata_pipeline():
    ...
    guesses = gen_data.guess_data(data, use_Iguess=False, gain_guess=False,
                                  perturbate=True, perturbate_factor=.5,
    ...
    ES = eigen_solver(data=data, guess=guesses, truth=truth,
                      lbfgs=False, conj_grad=True,
                        plot=True, plot_truth=True,  weights=weights,
